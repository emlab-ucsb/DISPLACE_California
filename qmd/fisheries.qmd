...
>**Note:** The data included in this repository and presented here are example data based on model output only. It is derived from the actual case study but does not represent real data, as the actual data inputs are confidential. The original data for this study consist of raw VMS records from the National Oceanic and Atmospheric Administration (NOAA) and landing receipts from the California Department of Fish and Wildlife (CDFW), covering the period 2010–2023.
...
The figure below shows example input fishing distribution data for our fleet and for an individual example vessel, broken down by the two metiers involved. These visualizations are based on model output, as the actual data inputs are confidential.
...
names(fe_distribution) <-c("Total fleet FE (h)", "DTS Trawl vessel FE (h)", "Non-DTS Trawl vessel FE (h)")
...
-   `mean_kW`: Mean engine power of the vessel. This variable influences the fuel consumption of simulated trips. In our case study, engine power values were only available for a few vessels. To estimate engine power for the rest, we used registry data from 2,043 vessels (@ODFW2024) and predicted engine power from tonnage and length using a model we developed.
...
knitr::include_graphics("figures/fuel_price_series.png")
```
...
knitr::include_graphics("figures/dts_fleet_selectivity.png")
```
