...
Below, we describe the contents of each required file.
...
where $W$ is weight at length $L$, and $a$ and $b$ are the parameters of the exponential relationship. The parameters are listed in @tbl-l-w-params. These values convert length (cm) to weight (kg).  

To convert length in centimeters to weight in grams, as required in DISPLACE, the parameter $a$ is multiplied by 1000.
...
Length–weight parameters are input in cm–g, and within the R routines they are transformed to cm–kg.
...
In this section, we detail the parametrization for our case study, to populate `Stock_biological_traits.csv`. We obtained the life history parameters from the most recent stock assessments available for each modeled species: the 2023 sablefish assessment (Johnson et al., 2023), the 2021 Dover sole assessment (Wetzel and Berger, 2021), the 2019 longspine thornyhead assessment (Adams et al., 2019), and the 2023 shortspine thornyhead assessment (Zahner et al., 2023). These parameters were extracted directly from the Stock Synthesis output files using the r4ss package (Taylor et al., 2021).
...
DISPLACE expects the stock–recruitment relationship to be specified using a **Beverton–Holt parameterization**. However the stock assessments assume the steepness form:
...
The natural mortality rates assumed by each assessment are provided in @tbl-mort-params below.
...
#| tbl-cap: Sample of the structure of the shapefile containing the species distributioin values

shp_list[[1]][,2:3] %>% head(10) %>% kable()
```
...
#| fig-cap: Species distribution in a continous scale from mean biomass values (kg/km2).
