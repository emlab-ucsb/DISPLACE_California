...
dir.create(file.path(
  ...
  paste("fishfarmsspe_", general$application, sep = '')
))
...
# read
fishfarms_features <- read.table(
  file.path(general$main_path_gis, "FISHFARMS", "fishfarms_features.csv"),
  sep = ";",
  header = TRUE
)
...
X <- ppp(
  x = an(fishfarms_features$long),
  y = an(fishfarms_features$lat),
  xrange = range(an(fishfarms_features$long)),
  yrange = range(an(fishfarms_features$lat))
)
Y <- ppp(
  x = coord_f[, "x"],
  y = coord_f[, "y"],
  xrange = range(coord_f[, "x"]),
  yrange = range(coord_f[, "y"])
)
...
weight_based_vbg <- function(Linf, K, t0, fulton_condition_factor, timestep) {
  return(
    ((fulton_condition_factor / 100000 * Linf^3) *
      (1 - exp(-K * (timestep - t0)))^3)
  )
}
meanw_grams_per_month <- weight_based_vbg(
  536.7,
  1.481,
  -0.274,
  2.10,
  seq(1, 5, by = 0.083333)
) # from end year 1 to end year 5 by month
...
variables_to_average <- c(
  ...
  "K_y",
  "t0_y",
  "fulton_condition_factor",
  "start_day_growing",
  "end_day_harvest",
  "nbyears_for_growth",
  "nb_days_fallowing_period",
  "meanw_at_start",
  "price_per_kg_at_start",
  "target_meanw_at_harvest",
  "meanw_at_harvest",
  "prop_harvest_kg_sold",
  "kg_eggs_per_kg",
  "feed_price_per_kg",
  "price_eggs_per_kg",
  "prop_N_in_feed",
  "prop_P_in_feed",
  "feed_vet_price_per_kg",
  "prop_N_in_feed_vet",
  "prop_P_in_feed_vet",
  "net_harvest_kg_per_sqkm_y",
  "market_price_sold_fish",
  "operating_cost_per_day"
)
ids <- paste(
  "list(",
  paste(
    paste("fishfarms_features[['", variables_to_average, "']]", sep = ""),
    collapse = ","
  ),
  ")"
)
fishfarms_features_mean <- aggregate(
  eval(parse(text = ids)),
  list(fishfarms_features$idx_node),
  mean
)
...
variables_to_sum <- c(
  ...
  "total_feed_vet_N_kg",
  "total_feed_vet_P_kg",
  "annual_discharge_N_kg",
  "annual_discharge_P_kg",
  "annual_discharge_C_kg",
  "annual_discharge_heavymetals_kg",
  "annual_discharge_medecine_kg",
  "annual_profit"
)
ids <- paste(
  "list(",
  paste(
    paste("fishfarms_features[['", variables_to_sum, "']]", sep = ""),
    collapse = ","
  ),
  ")"
)
fishfarms_features_sum <- aggregate(
  eval(parse(text = ids)),
  list(fishfarms_features$idx_node),
  sum
)
...
if (nrow(fishfarms_features) != nrow(fishfarms_features_sum)) {
  print(
    "Caution: some farms have to be merged! assumption on string var such as meanw_growth_model_type need to be made..."
  )
  some_fishfarms_features <- cbind.data.frame(
    data.frame::data.frame(
      idx = 1:nrow(fishfarms_features_mean),
      farm = paste0("farm", 1:nrow(fishfarms_features_mean)),
      meanw_growth_model_type = "((fulton_condition_factor/100000*Linf_mm^3)*(1-exp(-K_y*(tstep-t0_y)))^3)",
      feed_type = "BioMar_Ecolife_23",
      feed_type_vet = "AquaVet-OA"
    ),
    fishfarms_features_mean,
    fishfarms_features_sum
  )[, c('idx', 'farm', colnames(fishfarms_features[, -c(1, 2)]))]
} else {
...
some_fishfarms_features <- cbind.data.frame(
  some_fishfarms_features,
  is_active = 1
)

size_fishfarms_features <- some_fishfarms_features[, c("idx_node", "size_km2")] # This was missing in the original which made the UI to crash
...
# Saving missing file which was making the app to crash
write.table(
  size_fishfarms_features,
  file = file.path(
    general$main.path.ibm,
    paste("fishfarmsspe_", general$application, sep = ''),
