...
dir.create(file.path(
  ...
  paste("windmillsspe_", general$application, sep = '')
))
...
# read
windmills_features <- read.table(
  file.path(general$main_path_gis, "WINDMILLS", "windmills_features.csv"),
  sep = ";",
  header = TRUE
)
...
X <- ppp(
  x = an(windmills_features$long),
  y = an(windmills_features$lat),
  xrange = range(an(windmills_features$long)),
  yrange = range(an(windmills_features$lat))
)
Y <- ppp(
  x = coord_f[, "x"],
  y = coord_f[, "y"],
...
some_windmills_features <- tapply(
  some_windmills_features$size_km2,
  some_windmills_features$idx_node,
  sum
)
some_windmills_features <- cbind.data.frame(
  rownames(some_windmills_features),
  some_windmills_features
)
colnames(some_windmills_features) <- c('idx_node', 'size_km2')
