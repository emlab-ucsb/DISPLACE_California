...
dir.create(file.path(
  ...
  paste("benthosspe_", general$application, sep = '')
))
...
#!#!#!#!#!#!
#!#!#!#!#!#!
# name_GIS_file <- file.path("EunisMapsGreaterNS", "ModelledSeabedHabitats")
name_GIS_file <- file.path("habitat_landscapes")
#!#!#!#!#!#!
#!#!#!#!#!#!
#!#!#!#!#!#!
...
if (FALSE) {
  ...
  ## load shape.......
  landscapes <- readShapePoly(
    file.path(general$main_path_gis, "HABITATS", name_GIS_file),
    proj4string = CRS("+proj=longlat +datum=WGS84")
  ) # probably need an update of rgdal here....
  cat(paste(
    "Read GIS shape file (this one must include a CATEGORY field)...done\n"
  ))
...
if (FALSE) {
  # RASTER layer currently not handled by the DISPLACE Objects Editor
  #------------------
  # RASTER-----------
  #------------------
  library(maptools)
  ...
  coord <- cbind.data.frame(
    coord,
    landscape = paste(bottom_levels, photic_levels, salinity_levels, sep = "_")
  )

  # just for info...
  percent_habitats <- round(
    table(coord$landscape) / sum(table(coord$landscape)) * 100,
    2
  ) # get an idea of the importance of a given landscape in terms of number of nodes over the total number of nodes
...
if (
  !file.exists(file.path(
    general$main_path_gis,
    "HABITATS",
    "logistic_recovery_rates_per_month_per_funcgr.csv"
  ))
) {
  nbfuncgrps <- 4
  ...
  write.table(
    logistic_recovery_rates_per_month_per_funcgr,
    file = file.path(
      general$main.path.ibm,
      paste("benthosspe_", general$application, sep = ''),
      paste("logistic_recovery_rates_per_month_per_funcgr.dat", sep = '')
    ),
    col.names = TRUE,
    row.names = FALSE,
    quote = FALSE
  ...
  logistic_recovery_rates_per_month_per_funcgr <- recovery[, c(
    'landscape',
    'logistic_recovery_rate_per_month'
  )]
  write.table(
    logistic_recovery_rates_per_month_per_funcgr,
    file = file.path(
...
if (
  !file.exists(file.path(
    general$main_path_gis,
    "HABITATS",
    "benthos_biomass_carrying_capacity_K_per_landscape_per_funcgr.csv"
  ))
) {
  nbfuncgrps <- 4
  ...
  write.table(
    benthos_biomass_carrying_capacity_K_per_landscape_per_funcgr,
    file = file.path(
      general$main.path.ibm,
      paste("benthosspe_", general$application, sep = ''),
      paste(
  ...
  benthos_biomass_carrying_capacity_K_per_landscape_per_funcgr <- K[, c(
    'landscape',
    'benthos_biomass_carrying_capacity_K'
  )]
  write.table(
    benthos_biomass_carrying_capacity_K_per_landscape_per_funcgr,
    file = file.path(
...
if (
  !file.exists(file.path(
    general$main_path_gis,
    "HABITATS",
    "benthos_number_carrying_capacity_K_per_landscape_per_funcgr.csv"
  ))
) {
  nbfuncgrps <- 4
  ...
  write.table(
    benthos_number_carrying_capacity_K_per_landscape_per_funcgr,
    file = file.path(
      general$main.path.ibm,
      paste("benthosspe_", general$application, sep = ''),
      paste(
  ...
  benthos_number_carrying_capacity_K_per_landscape_per_funcgr <- K[, c(
    'landscape',
    'benthos_number_carrying_capacity_K'
  )]
  write.table(
    benthos_number_carrying_capacity_K_per_landscape_per_funcgr,
    file = file.path(
...
if (
  !file.exists(file.path(
    general$main_path_gis,
    "HABITATS",
    "prop_loss_on_habitat_after_one_passage_per_metier_per_sz.csv"
  ))
) {
  for (met in 0:max_met) {
    loss_after_one_passage_this_metier <- cbind.data.frame(
      landscape = rep(codes, each = nbgroups),
      loss_after_one_passage = -0.79
    )
    write.table(
      loss_after_one_passage_this_metier,
      file = file.path(
        general$main.path.ibm,
        paste("metiersspe_", general$application, sep = ''),
        paste(
  ...
  for (met in 0:max_met) {
    loss_after_one_passage_this_metier <- loss[
      loss$metier == met,
      c('landscape', 'loss_after_one_passage')
    ]
    write.table(
      loss_after_one_passage_this_metier,
      file = file.path(
...
if (
  !file.exists(file.path(
    general$main_path_gis,
    "HABITATS",
    "prop_funcgr_biomass_per_node_per_landscape.csv"
  ))
) {
  nbfuncgrps <- 4
  prop_funcgr_biomass_per_node_per_landscape <- cbind.data.frame(
    landscape = rep(codes, each = nbfuncgrps),
    prop_funcgr_per_node = 0.5
  ) # sum to 1 per node per landscape
  write.table(
    prop_funcgr_biomass_per_node_per_landscape,
    file = file.path(
      general$main.path.ibm,
      paste("benthosspe_", general$application, sep = ''),
      paste("prop_funcgr_biomass_per_node_per_landscape.dat", sep = '')
    ),
    col.names = TRUE,
    row.names = FALSE,
    quote = FALSE
  ...
  prop_funcgr_biomass_per_node_per_landscape <- read.table(
    file.path(
      general$main_path_gis,
      "HABITATS",
      "prop_funcgr_biomass_per_node_per_landscape.csv"
...
if (
  !file.exists(file.path(
    general$main_path_gis,
    "HABITATS",
    "prop_funcgr_number_per_node_per_landscape.csv"
  ))
) {
  nbfuncgrps <- 4
  prop_funcgr_number_per_node_per_landscape <- cbind.data.frame(
    landscape = rep(codes, each = nbfuncgrps),
    prop_funcgr_per_node = 0.5
  ) # sum to 1 per node per landscape
  write.table(
    prop_funcgr_number_per_node_per_landscape,
    file = file.path(
      general$main.path.ibm,
      paste("benthosspe_", general$application, sep = ''),
      paste("prop_funcgr_number_per_node_per_landscape.dat", sep = '')
    ),
    col.names = TRUE,
    row.names = FALSE,
    quote = FALSE
  ...
  prop_funcgr_number_per_node_per_landscape <- read.table(
    file.path(
      general$main_path_gis,
      "HABITATS",
      "prop_funcgr_number_per_node_per_landscape.csv"
...
if (
  !file.exists(file.path(
    general$main_path_gis,
    "HABITATS",
    "meanw_funcgr_per_landscape.csv"
  ))
) {
  nbfuncgrps <- 4
  meanw_funcgr_per_landscape <- cbind.data.frame(
    landscape = rep(codes, each = nbfuncgrps),
    meanweight_funcgr_per_node = 30
  ) # gram per sq meter
  write.table(
    meanw_funcgr_per_landscape,
    file = file.path(
      general$main.path.ibm,
      paste("benthosspe_", general$application, sep = ''),
      paste("meanw_funcgr_per_landscape.dat", sep = '')
    ),
    col.names = TRUE,
    row.names = FALSE,
    quote = FALSE
  ...
  meanw_funcgr_per_landscape <- read.table(
    file.path(
      general$main_path_gis,
      "HABITATS",
      "meanw_funcgr_per_landscape.csv"
...
if (
  !file.exists(file.path(
    general$main_path_gis,
    "HABITATS",
    "habitat_deltas_per_pop.csv"
  ))
) {
  # populate with 0 as default value
  habitat_deltas_per_pop <- cbind.data.frame(
    landscape = rep(codes, each = nbpops),
    habitat_deltas_per_pop = 0
  )
  # harvest function parameter for habitat effect (if 0=> no effect)
  write.table(
    habitat_deltas_per_pop,
    file = file.path(
      general$main.path.ibm,
      paste("benthosspe_", general$application, sep = ''),
      paste("habitat_deltas_per_pop.dat", sep = '')
    ),
    col.names = TRUE,
    row.names = FALSE,
    quote = FALSE
  ...
  habitat_deltas_per_pop <- read.table(
    file.path(general$main_path_gis, "HABITATS", "habitat_deltas_per_pop.csv"),
    header = TRUE,
    sep = ";"
  )
  write.table(
